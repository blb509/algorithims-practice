...
function organizingContainers(container) {
  // sum of containers never changes
  // total of a type equals total capacity of another container
  let containerTotals = [];
  let typeTotals = [];
  ...
  for (let i = 0; i < container.length; i++) {
    ...
    for (let j = 0; j < container[i].length; j++) {
      // finds container total by adding all types in one container
      containerTotal = containerTotal + container[i][j];
      // finds type total by adding one type from all containers
      typeTotal = typeTotal + container[j][i];
    }
    // add total to totals arrays
    containerTotals.push(containerTotal);
    typeTotals.push(typeTotal);
  }

  // sort each array so that you can determine if organzing is possible
  let containersSorted = containerTotals.sort((a, b) => a - b);
  let typesSorted = typeTotals.sort((a, b) => a - b);

  // check if the total of each type matches up with a total for a container
  for (let i = 0; i < containerTotals.length; i++) {
    if (containersSorted[i] != typesSorted[i]) {
      return "Impossible";
    }
  }
